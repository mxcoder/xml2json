...
pub(crate) fn read<R: BufRead>(mut reader: &mut Reader<R>, depth: u64, node_name: String) -> Value {
    let mut buf = Vec::new();
    let mut values = Vec::new();
    let mut node = Map::new();
    debug!("Parsing <{}> at depth: {}", node_name, depth);

    loop {
        match reader.read_event(&mut buf) {
            // Empty tags (no children tags, eg. `<image />`)
            Ok(Event::Empty(ref e)) => {
                if let Ok(name) = String::from_utf8(e.name().to_vec()) {
                    debug!("Processing empty tag <{}>", name);
            ...
            Ok(Event::Start(ref e)) => {
                if let Ok(name) = String::from_utf8(e.name().to_vec()) {
                    debug!("Processing non-empty tag <{}>", name);
                    let mut child = read(&mut reader, depth + 1, name.clone());
                    let mut attrs = Map::new();
                     ...
                     */
                    if !attrs.is_empty() {
                        debug!("Has attributes: {}", name);
                        if child.is_string() {
                            attrs.insert("#text".to_string(), child);
                        }

                        if let Ok(attrs) = serde_json::to_value(attrs) {
                            debug!("Only text attributes `{}`", name);
                            if node.contains_key(&name) {
                                debug!(
                                    "Node contains `{}` already, need to convert to array",
                                    name
                                );
                                let (_, mut existing) = node.remove_entry(&name).unwrap();
                                let mut entries: Vec<Value> = vec![];
                        ...
                        debug!("With attributes, no array {:?}", name);
